...
function Player() {
    ...
    function createPlayerTable() {
        ...
        for (const [idx, [name, is_dead]] of publicState.players_state.entries()) {
            array.push(<tr key={idx}>
                <th scope='row'>{idx}</th>
                {is_dead && <td><s>{name}</s></td>}
                {!is_dead && <td>
                    {idx === position && <ins> {name} </ins>}
                    {idx !== position && name}
                </td>}
                {is_dead === false && voteTarget !== idx && <td> <Button onClick={() => addMafiaVote(idx)}> Vote </Button></td>}
                {is_dead === false && voteTarget === idx && <td> <Button onClick={() => removeMafiaVote(idx)}> Vote </Button></td>}
                {is_dead === false && !targets.includes(idx) && <td> <Button onClick={() => addTarget(idx)}> Target </Button></td>}
                {is_dead === false && targets.includes(idx) && <td> <Button onClick={() => removeTarget(idx)}> Target </Button></td>}
            </tr>)
        };
        return array;
    };
